...
@Component({
  selector: 'app-calidad-edit',
  templateUrl: './calidad-edit.component.html'
})
export class CalidadEditComponent implements OnInit, DoCheck {

 /** Constantes a usar en el componente */
  constants = CONST_REGISTRO_DIARIO_CUADRILLA;
  constantsEstadoPeticion = CONST_REGISTRO_DIARIO_CUADRILLA_ESTADO_PETICION;
  CuadrillaCalidadModel: CuadrillaCalidadModel = new CuadrillaCalidadModel();
  clone: CuadrillaCalidadModel;
  ...
  constructor(@Inject(MAT_DIALOG_DATA) data: any, private formBuilder: FormBuilder, private snackBar: MatSnackBar, private dialogRef: MatDialogRef<CalidadEditComponent>, private serviceCuadrillaGeneral: CuadrillaGeneralService, private utilitiesServices: UtilitiesService, private commonService: CommonService, private dialog: MatDialog) {
    this.CuadrillaCalidadModel = JSON.parse(JSON.stringify(data.calidad));
  ...
  ngOnInit() {
    this.clone = JSON.parse(JSON.stringify(this.CuadrillaCalidadModel));
    ...
    // tslint:disable-next-line: max-line-length
    this.CuadrillaCalidadModel.tipoMaterial = this.CuadrillaCalidadModel.tipoMaterial === null ? undefined : this.CuadrillaCalidadModel.tipoMaterial;
    // tslint:disable-next-line: max-line-length
    this.CuadrillaCalidadModel.tipoEnsayo = this.CuadrillaCalidadModel.tipoEnsayo === null ? undefined : this.CuadrillaCalidadModel.tipoEnsayo;
    // tslint:disable-next-line: max-line-length
    this.CuadrillaCalidadModel.numeroMuestras = this.CuadrillaCalidadModel.numeroMuestras === null ? undefined : this.CuadrillaCalidadModel.numeroMuestras;
    // tslint:disable-next-line: max-line-length
    this.CuadrillaCalidadModel.resultado = this.CuadrillaCalidadModel.resultado === null ? undefined : this.CuadrillaCalidadModel.resultado;
  }
  ...
  onBack() {
    ...
    dialogRef.beforeClosed().subscribe(val => {
      if (val === 1) {
        this.CuadrillaCalidadModel = this.clone;
        this.close();
      }
    });
  }
