...
import { ObservacionesCreateComponent } from './observaciones-create/observaciones-create.component';
import { ObservacionesEditComponent } from './observaciones-edit/observaciones-edit.component';

@Component({
  selector: 'sigma-work-cuadrilla-observaciones',
  templateUrl: './work-cuadrilla-observaciones.component.html'
})
export class WorkCuadrillaObservacionesComponent implements OnInit {

  @Input() fourteenthFormGroup: FormGroup;
  ...
  CuadrillaObservacionGeneralModel: CuadrillaObservacionGeneralModel;
  ...
  */
  ...
  loadData(): void {
    ...
    this.CuadrillaObservacionGeneralModel = new CuadrillaObservacionGeneralModel();

    if (this.cuadrillaGeneral) {
      this.successInput = '1';
      this.dataCuadrillaMaterial = new MatTableDataSource(this.cuadrillaGeneral.observacionGeneral);
  ...
  add() {
    ...
    const dialogRef = this.dialog.open(ObservacionesCreateComponent, dialogConfig);

    if (this.petitionList) {
      this.petitionList.unsubscribe();
    }

    this.petitionList = this.serviceCuadrillaGeneral.serviceListenerObservaciones$.subscribe((data: CuadrillaObservacionGeneralModel) => {
   ...
   * que se va a editar
   */
  edit(observacion: CuadrillaObservacionGeneralModel, index: number) {
    const dialogConfig = new MatDialogConfig();
    dialogConfig.panelClass = 'edit-modalbox';
    dialogConfig.disableClose = true;
    dialogConfig.autoFocus = true;
    dialogConfig.data = {
      mantenimiento: this.mantenimiento,
      observacion: this.cuadrillaGeneral.observacionGeneral[index]
    };

    const dialogRef = this.dialog.open(ObservacionesEditComponent, dialogConfig);
   ...
   */
  delete(observacion: CuadrillaObservacionGeneralModel, index: number) {
    const dialogConfig = new MatDialogConfig();
    ...
    dialogRef.beforeClosed().subscribe(
      val => {
        if (val === 1) {
          this.cuadrillaGeneral.observacionGeneral.splice(index, 1);
          this.loadData();
          this.save(null, -1);
        }
      }
  ...
  save(data, index: number) {
    if (index !== -1) {
      this.cuadrillaGeneral.observacionGeneral[index] = data;
    } else if (data) {
      this.cuadrillaGeneral.observacionGeneral.push(data);
    }

    if (this.cuadrillaGeneral.id) {
      // tslint:disable-next-line: max-line-length
      this.saveSeccionCuadrilla.emit({ requestType: 'update', evento: 'loadData', owner: 'observaciones', nextStepper: false, cuadrillaGeneral: this.cuadrillaGeneral });
    } else {
      ...
      // tslint:disable-next-line: max-line-length
      this.saveSeccionCuadrilla.emit({ requestType: 'create', evento: 'loadData', owner: 'observaciones', nextStepper: false, cuadrillaGeneral: this.cuadrillaGeneral });
    }
  }
  ...
  onSubmit() {
    ...
    if (this.cuadrillaGeneral.observacionGeneral.length > 0) {
      this.successInput = '1';
