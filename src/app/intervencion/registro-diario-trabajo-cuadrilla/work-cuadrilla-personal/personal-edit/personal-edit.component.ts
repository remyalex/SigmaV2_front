...
export class PersonalEditComponent implements OnInit {
 ...
 /** Constantes a usar en el componente */
  ...
  cuadrillaPersonalModel: CuadrillaPersonalModel = new CuadrillaPersonalModel() ;
  clone: CuadrillaPersonalModel;
  ...
  constructor(@Inject(MAT_DIALOG_DATA) data: any, private formBuilder: FormBuilder, private snackBar: MatSnackBar, private dialogRef: MatDialogRef<PersonalEditComponent>, private serviceCuadrillaGeneral: CuadrillaGeneralService, private dialog: MatDialog) {
    this.mantenimiento = data.mantenimiento;
    this.cuadrillaGeneral = data.cuadrillaGeneral;
    this.cuadrillaPersonalModel = JSON.parse(JSON.stringify(data.persona));
    this.index = data.index;
    this.nombresApellidos = this.cuadrillaPersonalModel.persona.nombres + ' ' + this.cuadrillaPersonalModel.persona.apellidos;
  ...
  ngOnInit() {
    this.clone = JSON.parse(JSON.stringify(this.cuadrillaPersonalModel));
  ...
  onBack() {
    ...
    dialogRef.beforeClosed().subscribe(val => {
      if (val === 1) {
        this.cuadrillaPersonalModel = this.clone;
        this.close();
      }
    });
  }
  ...
  duplicateRecord(): boolean {
    ...
    for (const personal in this.cuadrillaGeneral.personal) {
      ...
      if (parseInt(personal) !== this.index) {
        // tslint:disable-next-line: max-line-length
