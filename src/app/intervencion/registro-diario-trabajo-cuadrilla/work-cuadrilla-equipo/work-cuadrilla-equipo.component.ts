...
@Component({
  selector: 'sigma-work-cuadrilla-equipo',
  templateUrl: './work-cuadrilla-equipo.component.html'
})
export class WorkCuadrillaEquipoComponent implements OnInit {
 ...
 /** Constantes a usar en el componente */
  ...
  CuadrillaEquipoModel: CuadrillaEquipoModel;
  ...
  */
  loadData(): void {
    ...
    this.CuadrillaEquipoModel = new CuadrillaEquipoModel();

    if (this.cuadrillaGeneral) {
      this.successInput = '1';
      this.dataCuadrillaMaterial = new MatTableDataSource(this.cuadrillaGeneral.equipos);
  ...
  add() {
    ...
    const dialogRef = this.dialog.open(EquipoCreateComponent, dialogConfig);

    if (this.petitionList) {
      this.petitionList.unsubscribe();
    }

    this.petitionList = this.serviceCuadrillaGeneral.serviceListenerEquipo$.subscribe((data: CuadrillaEquipoModel) => {
   ...
   * que se va a editar
   */
  edit(equipo: CuadrillaEquipoModel, index: number) {
    ...
    dialogConfig.data = {
      mantenimiento: this.mantenimiento,
      equipo: this.cuadrillaGeneral.equipos[index]
    };
   ...
   */
  delete(equipo: CuadrillaEquipoModel, index: number) {
    const dialogConfig = new MatDialogConfig();
    ...
    dialogRef.beforeClosed().subscribe(
      val => {
        if (val === 1) {
          this.cuadrillaGeneral.equipos.splice(index, 1);
          this.loadData();
          this.save(null, -1);
        }
      }
  ...
  save(data, index: number) {
    if (index !== -1) {
      this.cuadrillaGeneral.equipos[index] = data;
    } else if (data) {
      this.cuadrillaGeneral.equipos.push(data);
    }

    if (this.cuadrillaGeneral.id) {
      // tslint:disable-next-line: max-line-length
      this.saveSeccionCuadrilla.emit({ requestType: 'update', evento: 'loadData', owner: 'equipos', nextStepper: false, cuadrillaGeneral: this.cuadrillaGeneral });
    } else {
      ...
      // tslint:disable-next-line: max-line-length
      this.saveSeccionCuadrilla.emit({ requestType: 'create', evento: 'loadData', owner: 'equipos', nextStepper: false, cuadrillaGeneral: this.cuadrillaGeneral });
    }
  }
