...
import { ResumenDirectorObraService } from './resumen.director.obra.service';
 ...
 **/
export class ResumenDirectorObraDatasource implements DataSource<ResumenDirectorObraModel> {  
 /** Constantes a usar en el componente */
  constants = CONST_WORKFLOW_DIAGNOSTICO;
  private programarVisitaVerificacionSubject = new BehaviorSubject<ResumenDirectorObraModel[]>([]);
  ...
  public resumenDirectorObraData: any;
  ...
  */
  constructor (private servicio: ResumenDirectorObraService) { }
   ...
   */
  connect(collectionViewer: CollectionViewer): Observable<ResumenDirectorObraModel[]> {
    return this.programarVisitaVerificacionSubject.asObservable();
  }
  ...
  disconnect(collectionViewer: CollectionViewer): void {
    this.programarVisitaVerificacionSubject.complete();
 ...
 /**
   ...
   */
  loadData(criteria: ResumenDirectorObraCriteria): void {
    ...
    this.petitionList = this.servicio.search(criteria)
      .subscribe(
        async (data: any) => {
          this.resumenDirectorObraData = data.content;
          this.programarVisitaVerificacionSubject.next(data.content);
        ...
        error => {
          this.resumenDirectorObraData = [];
          this.programarVisitaVerificacionSubject.next([]);
