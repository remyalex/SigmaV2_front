...
@Component({
  ...
  selector: 'sigma-predisenio-edit',
  templateUrl: './predisenio-edit.component.html'
})
export class PredisenioEditComponent implements OnInit, OnDestroy {
  ...
  */
  ...
  ngOnInit() {
    ...
    if (this.mantenimiento.tipoSuperficie.descripcion === 'RÍGIDO' && this.mantenimiento.actividadAgrupada === 'RH') {
      ...
      this.predisenio.modulacionLosas = true;
      this.predisenio.levantamientoTopografico = true;
    }

    for (const key in this.predisentioEdit) {
      if (this.predisentioEdit[key] !== this.predisenio[key]) {
        this.predisenio[key] = null;
      }
    }
  }
  ...
  ubicarApique(): void {
    ...
    dialogRef.beforeClosed().subscribe(
      val => {
        if (val) {
          this.unsubscribe$ = this.servicio.serviceListener$.subscribe((data: any) => {
            this.ubicarAction = 1;
            this.validarApiques = true;
            this.predisenio.apiques = data.predisenio.apiques;
