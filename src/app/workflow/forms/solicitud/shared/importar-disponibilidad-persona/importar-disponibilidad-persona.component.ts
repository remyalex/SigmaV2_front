...
import { CONST_ADMINISTRACION_PERSONADISPONIBILIDAD } from './../../../../../administracion/personadisponibilidad/personadisponibilidad.constant';
 ...
 * de la disponibilidad para las personas */
@Component({
  selector: 'app-importar-disponibilidad-persona',
  templateUrl: './importar-disponibilidad-persona.component.html'
})
export class ImportarDisponibilidadPersonaComponent implements OnInit {
  ...
  /** Modelo de disponibilidad de la persona a importar */
  ...
  displayedColumns: string[] = ['data.identificacion' , 'data.fechaDesde', 'data.fechaHasta', 'error', 'message'];
   ...
   */
  constructor(
    private dialogRef: MatDialogRef<ImportarDisponibilidadPersonaComponent>,
  ...
  import() {
    ...
    this.importarExcelModel.tipoImportacion = 'DISPONIBILIDAD_PERSONA';
  ...
  descargarFormato() {
    this.processing = true;
    this.commonService.downloadFormato('DISPONIBILIDAD_PERSONA').subscribe(data => {
      const body = data;
      this.processing = false;
      ...
      a.href = url;
      a.download = 'formatoDisponibilidadPersona.xlsx';
      a.click();
      setTimeout(() => {
