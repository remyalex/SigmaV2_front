 ...
 * de la disponibilidad para los equipos */
@Component({
  selector: 'app-importar-disponibilidad-equipo',
  templateUrl: './importar-disponibilidad-equipo.component.html'
})
export class ImportarDisponibilidadEquipoComponent implements OnInit {
  ...
  /** Modelo de disponibilidad del equipo a importar */
  equipoDisponibilidad: RecursoDisponiblidadInfo;
  ...
  displayedColumns: string[] = ['data.numeroInterno', 'data.desde', 'data.hasta', 'error', 'message'];
 ...
 /**
   ...
   */
  constructor(
    private dialogRef: MatDialogRef<ImportarDisponibilidadEquipoComponent>,
  ...
  import() {
    ...
    this.importarExcelModel.tipoImportacion = 'DISPONIBILIDAD_EQUIPO';
  ...
  descargarFormato() {
    this.processing = true;
    this.commonService.downloadFormato('DISPONIBILIDAD_EQUIPO').subscribe(data => {
      this.processing = false;
      const body = data;
      ...
      a.href = url;
      a.download = 'formatoDisponibilidadEquipo.xlsx';
      a.click();
      setTimeout(() => {
