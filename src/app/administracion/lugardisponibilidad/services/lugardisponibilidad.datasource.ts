 ...
 **/
export class LugardisponibilidadDatasource implements DataSource<Lugardisponibilidad> {
 /** Constantes a usar en el componente */
  constants = CONST_ADMINISTRACION_LUGARDISPONIBILIDAD;
  private lugardisponibilidadSubject = new BehaviorSubject<Lugardisponibilidad[]>([]);
  ...
  public lugardisponibilidadData: any;
  ...
  */
  constructor (private servicio: LugardisponibilidadService) { }
   ...
   */
  connect(collectionViewer: CollectionViewer): Observable<Lugardisponibilidad[]> {
    return this.lugardisponibilidadSubject.asObservable();
  }
  ...
  disconnect(collectionViewer: CollectionViewer): void {
    this.lugardisponibilidadSubject.complete();
 ...
 /**
   ...
   */
  loadData(criteria: LugardisponibilidadCriteria): void {
    ...
    this.petitionList = this.servicio.search(criteria)
      .subscribe(async (data: any) => {
        const datosNuevos = data.content.map(async (lugardisponibilidad) => {
          return {
            ...lugardisponibilidad
          };
        });
        Promise.all(datosNuevos)
          .then((completed) => {
            ...
            this.lugardisponibilidadData = completed;

            this.lugardisponibilidadSubject.next(data.content);
        ...
        error => {
          this.lugardisponibilidadSubject.next([]);
