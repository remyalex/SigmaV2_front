 ...
 **/
export class EventoDatasource implements DataSource<Evento> {
  ...
  */
   ...
   */
  connect(collectionViewer: CollectionViewer): Observable<Evento[]> {
   ...
   */
  loadData(criteria: EventoCriteria ): void {
      ...
      this.petitionList = this.servicio.search(criteria)
          .subscribe(async(data: any) => {
            const datosNuevos = data.content.map(async (evento) => {
              return {...evento,
              ...{
              }};
            });
            ...
            .then((completed) => {
              ...
              this.eventoData = completed;
              this.eventoSubject.next(data.content);
