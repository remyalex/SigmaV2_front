 ...
 **/
export class PersonanovedadDatasource implements DataSource<Personanovedad> {

 /** Constantes a usar en el componente */
  constants = CONST_ADMINISTRACION_PERSONANOVEDAD;
  private personanovedadSubject = new BehaviorSubject<Personanovedad[]>([]);
  ...
  */
  constructor (private servicio: PersonanovedadService){}
   ...
   */
  connect(collectionViewer: CollectionViewer): Observable<Personanovedad[]> {
    return this.personanovedadSubject.asObservable();
  }
  ...
  disconnect(collectionViewer: CollectionViewer): void {
    this.personanovedadSubject.complete ();
   ...
   */
  loadData(criteria: PersonanovedadCriteria ): void {
      ...
      this.petitionList = this.servicio.search(criteria)
          .subscribe(async (data: any) => {
            const datosNuevos = data.content.map(async (personanovedad) => {
              const novedadpersona_tipo = await this.servicio.searchByList(
                this.constants.path_administracion_personanovedad_tipoNovedadPersonaId, personanovedad.tipoNovedadPersonaId);
              return {
                ...personanovedad,
                ...{
                  // tslint:disable-next-line:max-line-length
                  tipoNovedadPersonaValor: (typeof novedadpersona_tipo !== 'undefined') ? novedadpersona_tipo.valor : this.constants.noEncontrado
                }
              };
            });
            ...
            .then((completed) => {
              ...
              this.personanovedadData = completed;
              this.personanovedadSubject.next(data.content);
          ...
          error => {
            this.personanovedadSubject.next([]);
