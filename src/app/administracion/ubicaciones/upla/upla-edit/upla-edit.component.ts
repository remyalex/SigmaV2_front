...
/** Componente encargado de gestionar la edición de una upla*/
@Component({
  selector: 'app-upla-edit',
  templateUrl: './upla-edit.component.html'
})
export class UplaEditComponent implements OnInit {
 ...
 /** Constantes a usar en el componente */
  ...
  constructor(
    ...
    private dialogRef: MatDialogRef<UplaEditComponent>,
  ...
  ngOnInit() {
    this.clone = JSON.parse(JSON.stringify(this.upla));
  }
  ...
  close () {
    ...
    dialogRef.beforeClosed().subscribe(val => {
      if (val === 1) {
        for (const key in this.upla) {
          this.upla[key] = this.clone[key];
        }
  ...
  save() {
    ...
    if (this.form.valid === true) {
      this.servicio.update(this.upla).subscribe(data => {
        this.dialogRef.close();
