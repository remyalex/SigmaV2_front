 ...
 **/
export class PersonadisponibilidadDatasource implements DataSource<Personadisponibilidad> {
  /** Constantes a usar en el componente */
  constants = CONST_ADMINISTRACION_PERSONADISPONIBILIDAD;
  private personadisponibilidadSubject = new BehaviorSubject<Personadisponibilidad[]>([]);
  ...
  public personadisponibilidadData: any;
  ...
  */
  constructor (private servicio: PersonadisponibilidadService) { }
   ...
   */
  connect(collectionViewer: CollectionViewer): Observable<Personadisponibilidad[]> {
    return this.personadisponibilidadSubject.asObservable();
  }
  ...
  disconnect(collectionViewer: CollectionViewer): void {
    this.personadisponibilidadSubject.complete();
  ...
  reloadData(listItems:any){
    this.personadisponibilidadData = listItems;
    this.personadisponibilidadSubject.next(listItems);
  }
 ...
 /**
   ...
   */
  loadData(criteria: PersonadisponibilidadCriteria): void {
    ...
    this.petitionList = this.servicio.search(criteria)
      .subscribe(async (data: any) => {
        const datosNuevos = data.content.map(async (personadisponibilidad) => {
          return {
            ...personadisponibilidad,
            ...{ }
          };
        });
        Promise.all(datosNuevos)
          .then((completed) => {
            ...
            this.personadisponibilidadData = completed;

            this.personadisponibilidadSubject.next(data.content);
        ...
        error => {
          this.personadisponibilidadSubject.next([]);
