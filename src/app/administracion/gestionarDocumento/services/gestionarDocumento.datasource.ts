 ...
 **/
export class GestionarDocumentosource implements DataSource<GestionarDocumento> {
 /** Constantes a usar en el componente */
  constants = CONST_ADMINISTRACION_GESTIONAR_DOCUMENTO;
  private gestionarDocumentoSubject = new BehaviorSubject<GestionarDocumento[]>([]);
  ...
  public gestionarDocumentoData: any;
  ...
  */
  constructor (private servicio: GestionarDocumentoService) { }
   ...
   */
  connect(collectionViewer: CollectionViewer): Observable<GestionarDocumento[]> {
    return this.gestionarDocumentoSubject.asObservable();
  }
  ...
  disconnect(collectionViewer: CollectionViewer): void {
    this.gestionarDocumentoSubject.complete();
 ...
 /**
   ...
   */
  loadData(criteria: GestionarDocumentoCriteria): void {
    ...
    this.petitionList = this.servicio.search(criteria)
      .subscribe(async (data: any) => {
        const datosNuevos = data.content.map(async (documento) => {
          return {
            ...documento,
            ...{
            }
        ...
        Promise.all(datosNuevos)
          .then((completed) => {
            ...
            this.gestionarDocumentoData = completed;
            this.gestionarDocumentoSubject.next(data.content);
          });
        this.gestionarDocumentoData = data.content;
        this.gestionarDocumentoSubject.next(data.content);
        ...
        error => {
          this.gestionarDocumentoSubject.next([]);
