...
import { MatTableDataSource, MatDialogConfig, MatSnackBar, MatDialog, MatPaginator } from '@angular/material';
import { SigmaConfirmComponent } from 'src/app/shared/sigma-confirm/sigma-confirm.component';
import { RouterModule, Router } from '@angular/router';
import { UtilitiesService } from 'src/app/shared/services/utilities.service';

/** Componente encargado de gestionar la creación de las condiciones en las transiciones*/
@Component({
  selector: 'app-transicioncondiciones-create',
  templateUrl: './transicioncondiciones-create.component.html'
})
export class TransicioncondicionesCreateComponent implements OnInit {
  ...
  hideValor = true;
  /** Bandera que permite identificar si la sección para el operador IN/NOT IN está oculta */
  hideCamposIN = true;
  /** Bandera que permite identificar si el boton enviar está desabilitado */
  disabledAddButton = true;
  /** Bandera que permite identificar si el campo valor esta desabilitado */
  disabledValor = false;
  ...
  constructor(
    ...
    this.condicion = new Condiciones();
  ...
  */
  ...
  setAtributo(atributo: any) {
    ...
    if (atributo) {
      if (atributo.tipo === 'LISTA_TIPO_INTERVENCION' || atributo.tipo === 'LISTA_ACTIVIDAD' || atributo.tipo === 'LISTA_ITEM' || atributo.tipo === 'BOOLEAN' || atributo.tipo === 'NVARCHAR2' || atributo.tipo === 'VARCHAR2') {
  ...
  */
  ...
  add() {
    ...
    this.condicion.terminos.push(JSON.parse(JSON.stringify(this.termino)));
  ...
  save() {
    if (this.condicion.terminos.length === 0) {
      this.snackBar.open('¡No puede crear una condicion sin terminos!', 'X', {
        duration: 5000,
        panelClass: ['warning-snackbar']
      });
      this.disableSubmit = false;
    } else {
      this.service.create(this.condicion).subscribe(
        data => {
