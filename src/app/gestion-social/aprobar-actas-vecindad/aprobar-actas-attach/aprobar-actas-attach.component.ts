...
@Component({
    selector: 'app-aprobar-actas-attach',
    templateUrl: './aprobar-actas-attach.component.html'
})
export class AprobarActasAttachComponent extends BaseComponent implements OnInit, AfterViewChecked, FormComponent {
    ...
    constructor(
        ...
        for (var i = 0; i < this.data.mantenimiento.actasVecindad.length; i++) {
            if (this.data.mantenimiento.actasVecindad[i].id == CONST_APROBAR_ACTAS.aID) {
                this.index = i;
            }
        }
