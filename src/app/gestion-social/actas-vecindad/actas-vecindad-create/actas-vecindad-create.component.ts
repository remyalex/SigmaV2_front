import { Component, OnInit, Inject, AfterViewInit, Input, Output, EventEmitter, AfterViewChecked } from '@angular/core';
...
@Component({
    selector: 'app-actas-vecindad-create',
    templateUrl: './actas-vecindad-create.component.html'
})
export class ActasVecindadCreateComponent extends BaseComponent implements OnInit, AfterViewChecked, FormComponent {
    ...
    registroNomenclaturaDatasource: MatTableDataSource<ActaArchivos> = new MatTableDataSource<ActaArchivos>();
    registroFachadaDatasource: MatTableDataSource<ActaArchivos> = new MatTableDataSource<ActaArchivos>();
    registroFotograficoDatasource: MatTableDataSource<ActaArchivos> = new MatTableDataSource<ActaArchivos>();
    ...
    ngOnInit() {
        ...
        var f = new Date();
        if (!this.data.mantenimiento.actasVecindad) {
            this.data.mantenimiento.actasVecindad = [];
        }

        this.data.mantenimiento.actasVecindad.push(new Acta());
        this.index = this.data.mantenimiento.actasVecindad.length - 1;
        this.acta.fecha = f.getDate() + "-" + (f.getMonth() + 1) + "-" + f.getFullYear();
        this.acta.noActaVecindad = this.data.mantenimiento.civ + '-' + (this.data.mantenimiento.actasVecindad.length).toString().padStart(4, "0");

        this.subscription = this.actasService.detailActividad(40).subscribe(data => {
            this.actividad = data;
        }, error => {
            console.log(error);
        });
