...
import { ActaAficheArchivoModel } from '../models/acta-afiche-archivo.model';

@Component({
  selector: 'app-acta-afiche-create',
  templateUrl: './acta-afiche-create.component.html'
})
export class ActaAficheCreateComponent extends BaseComponent implements OnInit {
 ...
 /** Constantes a usar en el componente */
  ...
  ngOnInit() {
    ...
    this.actaAfiche.actaAficheArchivo = new ActaAficheArchivoModel();
    ...
    if (pk !== null && pk.length > 0) {
      this.servicio.detailByPk(Number(pk)).subscribe(data => {
        ...
        this.actaAfiche.actaAficheArchivo.fechaRegistro = this.utilitiesServices.convertDateToString(new Date(), 'DD-MM-YYYY');
  ...
  save() {
    ...
    const consecutivo = this.mantenimiento.intervenciones[0].actasAfiche.length + 1;
    if (consecutivo > 999) {
      this.actaAfiche.noAfiche = this.mantenimiento.civ + '-' + consecutivo;
    } else if (consecutivo > 99) {
      this.actaAfiche.noAfiche = this.mantenimiento.civ + '-0' + consecutivo;
    } else if (consecutivo > 9) {
      this.actaAfiche.noAfiche = this.mantenimiento.civ + '-00' + consecutivo;
    } else {
      this.actaAfiche.noAfiche = this.mantenimiento.civ + '-000' + consecutivo;
    }
    this.utilitiesServices.scrollToTop();
    this.servicioActa.create(this.actaAfiche).subscribe(
      data => {
