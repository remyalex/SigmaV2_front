 ...
 **/
export class RegistroMezclaInsumoDatasource implements DataSource<SolicitudMezclaInsumos>{
 /** Constantes a usar en el componente */
  constants = CONST_REGISTRAR_MEZCLA_INSUMOS;
  private ensayosSubject = new BehaviorSubject<SolicitudMezclaInsumos[]>([]);
  ...
  */
  constructor (private servicio: RegistroMezclaInsumosService) { }
   ...
   */
  connect(collectionViewer: CollectionViewer): Observable<SolicitudMezclaInsumos[]> {
 ...
 /**
   ...
   */
  loadData(criteria: RegistroMezclaInsumoCriteria, payload): void {
  ...
  loadData_list(criteria: RegistroMezclaInsumoCriteria, payload, id, material): void {
    this.loadingSubject.next(true);
    if (this.petitionList) {
      this.petitionList.unsubscribe();
    }
    this.petitionList = this.servicio.listSolicitudConsultar(criteria, payload, id, material)
    .subscribe(
        async (data: any) => {
          this.ensayosData = data;
          this.ensayosSubject.next(data.content);
  ...
  loadData_detalles(criteria: RegistroMezclaInsumoCriteria, payload): void {
    this.loadingSubject.next(true);
    if (this.petitionList) {
      this.petitionList.unsubscribe();
    }
    this.petitionList = this.servicio.listDetallesVales(criteria, payload)
    .subscribe(
        async (data: any) => {
          this.ensayosData = data;
          this.ensayosSubject.next(data.content);
