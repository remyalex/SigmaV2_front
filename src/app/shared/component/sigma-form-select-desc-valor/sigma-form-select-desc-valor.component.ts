...
/**
 * Componente usado para estandarizar el campo de selección por valor descendente
 * en todos los formularios del sistema
 */
@Component({
  selector: 'sigma-form-select-desc-valor',
  templateUrl: './sigma-form-select-desc-valor.component.html',
  providers: [
    {
      provide: MatFormFieldControl,
      useExisting: SigmaFormSelectDescValorComponent
    }
  ],
  changeDetection: ChangeDetectionStrategy.OnPush
})

export class SigmaFormSelectDescValorComponent implements OnInit, ControlValueAccessor, DoCheck {
  ...
  @Input('orderBy') orderBy: any = 'valor';
  ...
  getOptions() {
    if (this.opciones.length > 0) {
      ...
      this.servicioGeneral.listQuery$
        ...
        .subscribe((data: any) => {
          try {
            data.content = this.convertValorStringtoNumeric(data.content);
            let opciones = this.utilitiesService.orderArray(data.content, this.orderBy);
  ...
  /** Método encargado de recorrer una lista, validar si el campo 'valor' es 
   * de tipo String y convertirlo a Numeric
   */
  convertValorStringtoNumeric(data: any) {
    if (this.orderBy === 'valor') {
      for (let obj of data) {
