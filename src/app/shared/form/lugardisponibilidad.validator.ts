...
export class LugardisponibilidadValidator {

    static datosNecesarios(grupo: FormGroup) {
        if (grupo.controls.hasOwnProperty('lugar')) {
            let lugar = grupo.get('lugar').value;
            if(lugar){
                if(!lugar.fechaDesde || !lugar.fechaHasta || !lugar.horaFin || !lugar.horaInicio){
                    grupo.get('lugar').setErrors({ errorDatosNecesariosLugar: true });
                    return null;
                }
            }
            
            grupo.get('lugar').setErrors(null);
            return null;
        }
